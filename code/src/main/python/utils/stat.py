...
class Stat(lib.O):
  ...
  def report(self):
    try:
      return "\n".join(["```",
                        "\n".join(["%10s : %0.8f" % ("Min", self.min()),
                                   "%10s : %0.8f" % ("Q25", self.q25()),
                                   "%10s : %0.8f" % ("Median", self.median()),
                                   "%10s : %0.8f" % ("Q75", self.q75()),
                                   "%10s : %0.8f" % ("Max", self.max()),
                                   "%10s : %0.8f" % ("Mean", self.mean()),
                                   "%10s : %0.8f" % ("Std. Dev.", self.std()),
                                   "%10s : %0.8f" % ("IQR", self.iqr()),
