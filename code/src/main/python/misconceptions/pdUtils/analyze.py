...
from analysis.helpers import ast_utils, execution_utils, helper
...
def create_temp_function_file(variable, body):
  ...
  if func_node is None:
    return None
  func_str = ast_utils.convert_ast_to_code(func_node)
...
def strip_comments(code):
  tree = ast.parse(code)
  return ast_utils.convert_ast_to_code(tree).strip()
...
def execute_stmt(stmt, args):
  ...
  for ret, body in ret_bodies.items():
    ...
    results = []
    for arg_set in args:
      result = execution_utils.execute_function(func, arg_set)
      try:
