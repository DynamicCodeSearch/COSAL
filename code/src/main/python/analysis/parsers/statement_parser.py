...
class StatementVisitor(parser.Traveller):
  def __init__(self, file_path, dataset, infer_types=False, force_fetch=False):
    ...
    if infer_types:
      # TODO: Create class to infer types
      variable_visitor = arg_analysis.parse_file_via_inference(file_path, dataset, force_fetch)
    else:
      variable_visitor = arg_analysis.parse_file_for_args(file_path, dataset)
    # self.statement_blocks = []
    self.statement_groups = []
    self.classes = variable_visitor.classes
  ...
  def set_root_method(self):
    start, end = position_block.Position.get_start_end(self.ast_tokenized.tree)
  ...
  def visit_FunctionDef(self, node, meta):
    ...
    if not self.is_valid_scope(self._current_scope):
      ...
      return
    start, end = position_block.Position.get_start_end(node)
  ...
  def visit_ClassDef(self, node, meta):
    ...
    self._current_scope = scope_block.Scope(node.name, self._current_scope)
    for child in node.body:
      self.visit(child, meta)
    self._current_scope = self._current_scope.parent
