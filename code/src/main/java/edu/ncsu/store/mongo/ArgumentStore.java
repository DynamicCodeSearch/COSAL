...
public class ArgumentStore implements IArgumentStore {
    ...
    public void savePrimitiveArguments(Map<Primitive, Set<Object>> primitiveArguments) {
        LOGGER.info("Saving Primitive Arguments into MongoDB ... ");
        Map<Primitive, Set<Object>> existingPrimitiveArguments = loadPrimitiveArguments();
        List<Document> documents = new ArrayList<>();
        for (Primitive primitive: primitiveArguments.keySet()) {
            Set<Object> args = primitiveArguments.get(primitive);
            if (existingPrimitiveArguments.containsKey(primitive))
                args.addAll(existingPrimitiveArguments.get(primitive));
            Document document = new Document("type", primitive.getName())
                    .append("family", primitive.getFamily().getName())
                    .append("args", args);
    ...
    public Set<Object> loadPrimitiveArgumentsForName(Primitive primitive) {
        MongoCollection<Document> collection = getPrimitiveCollection();
        Document doc = MongoDriver.getDocument(collection, "type", primitive.getName());
        if (doc != null)
            return new HashSet<Object>((List) doc.get("args"));
//        throw new RuntimeException(String.format("Looks like you have not extracted sample primitives from the the primitive: %s", primitive.getName()));
        return null;
    }
