...
public class MethodBlock {
    ...
    public MethodBlock(MethodDeclaration methodDeclaration, String fileSource, String parentClass) {
        ...
        if (methodDeclaration.getBegin().isPresent()) {
            this.startLine = methodDeclaration.getBegin().get().line;
            this.startColumn = methodDeclaration.getBegin().get().column;
        }
        if (methodDeclaration.getEnd().isPresent()) {
            this.endLine = methodDeclaration.getEnd().get().line;
            this.endColumn = methodDeclaration.getEnd().get().column;
        }
        this.methodNode = methodDeclaration;
    }

    public boolean isStatic() {
        return VisitorHelper.isStatic(this.methodNode.getModifiers());
    }
...
//        System.exit(0);
    ...
    public List<StatementBlock> getStatements() {
        ...
        if (!this.methodNode.getBody().isPresent())
            return null;
        for (Statement stmt : this.methodNode.getBody().get().getStatements()) {
            StatementBlock stmtBlock = StatementHelper.parseStatementNode(fileSource, parentClass,
                    methodNode.getNameAsString(), stmt, null);
            if (stmtBlock != null)
    ...
    private List<List<StatementBlock>> getStatementGroups(List<StatementBlock> statementBlocks) {
        ...
        if (statementBlocks == null
//                || statementBlocks.size() < HyperParameters.MIN_STATEMENT_SIZE
...
//                retList.add(sBlocks);
        ...
        if (statementGroup.size() >= HyperParameters.MIN_STATEMENT_SIZE) {
    ...
    private List<List<StatementBlock>> getCombinations(List<StatementBlock> statementGroup) {
        ...
        for (int step_size = HyperParameters.MIN_STATEMENT_SIZE; step_size < statementGroup.size(); step_size++) {
